...
Every page renders through `src/layouts/BaseLayout.astro`, which emits the
canonical URL, Open Graph and Twitter card tags, the favicon links, and any
JSON-LD passed via the `schema` prop. Structured-data builders live in
`src/lib/schema.ts` — `Organization` and `WebSite` on the home page,
`Service` + `BreadcrumbList` on each practice-area page, `BlogPosting` +
`BreadcrumbList` on posts.

**Indexing is environment-aware, driven by the same `PUBLIC_SITE_URL` switch
as drafts** (`src/lib/site.ts`):

| Build | `robots.txt` | Per-page `<meta name="robots">` |
|---|---|---|
| Production (`PUBLIC_SITE_URL` = `https://resultsmatrix.com`) | `Allow: /`, disallows `/admin/`, links the sitemap | `index, follow` |
| Staging / local (anything else) | `Disallow: /` | `noindex, nofollow` |

This is what keeps draft posts on staging out of search results. Individual
pages can opt out in production with the layout's `noindex` prop (the 404
page and draft posts do).

`sitemap-index.xml` / `sitemap-0.xml` are generated by `@astrojs/sitemap` at
build time. `/privacy/` and the `/blog/tags/` archives are excluded (see the
`filter` in `astro.config.mjs`); everything else is listed.

Internal links must end in a trailing slash (`/contact/`, not `/contact`) —
`trailingSlash: 'always'` makes that the canonical form, and links without it
cost a redirect hop.

**Favicons.** `public/favicon.*`, `apple-touch-icon.png`, `icon-*.png`, and
`site.webmanifest` are generated from `src/assets/logo/rml-mark.svg`. After
changing the mark, regenerate and commit the output:

```sh
node scripts/generate-favicons.mjs
```

**Auditing.** Lighthouse must stay ≥ 95 on all four categories for the home
page and a blog post (BRIEF.md §8). Run it against a production-flavored
build — note `--host 127.0.0.1`, without which the preview server is not
reachable by the Lighthouse Chrome instance:

```sh
PUBLIC_SITE_URL=https://resultsmatrix.com npm run build
npx astro preview --port 4399 --host 127.0.0.1 &
npx lighthouse http://127.0.0.1:4399/ --preset=desktop --view
```

Total Blocking Time varies noticeably between runs on a local preview (no
HTTP/2, no compression); take the median of three before acting on a drop.
